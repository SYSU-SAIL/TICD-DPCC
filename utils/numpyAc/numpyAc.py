...
def _encode_float_cdf(cdf_float,
    ...
    """
    float CDF 编码
    :param cdf_float: 概率分别函数 Shape (N1, ..., Nm, Lp).
    :param sym: 需要编码的符号 int16, on CPU. Shape (N1, ..., Nm).
    :param needs_normalization: 是否需要 normalization 如果为 True，则假设“cdf_float”未标准化，并且需要正常化。
    否则仅对其进行转换，而不进行标准化。
    :param check_input_bounds: 如果为 True，请确保输入具有有效值。
      重要提示：可能需要大量时间。仅启用检查。
    :return: 字节字符串，编码“sym”。
    """
